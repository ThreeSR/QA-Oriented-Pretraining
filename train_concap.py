...
def main():
    ...
    if dist.is_available() and args.local_rank != -1:
        rank = dist.get_rank() # torch.distributed = dist
        if rank == 0:
            default_gpu = True
    else:
        default_gpu = True
    ...
    if default_gpu:
        if not os.path.exists(savePath):
            os.makedirs(savePath) # 创建输出文件夹

    config = BertConfig.from_json_file(args.config_file)

    if default_gpu:
        # save all the hidden parameters.
        with open(os.path.join(savePath, "command.txt"), "w") as f: # 记录实验参数，这个是在save文件夹下面的内容
            print(args, file=f)  # Python 3.x
            print("\n", file=f)
            print(config, file=f)

    args.train_batch_size = args.train_batch_size // args.gradient_accumulation_steps # 不应该相乘？？
    ...
    # 后面代码就是使用下面处理好的数据，所以重点在于下面数据处理代码的改写
    # file_path就是cc的数据路径
    ...
    for epochId in range(int(args.start_epoch), int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataset):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # 日志文件也需要改写！
                if default_gpu: # 下面是画图的代码，记录在tensorboard之中
            ...
            if (
                ...
                tbLogger.showLossTrainCC() # 画图记录数据
