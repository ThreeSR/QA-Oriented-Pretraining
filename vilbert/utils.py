...
class MultiTaskStopOnPlateau(object): # 如果不使用step方法进行更新，那么永远都会在go，就是正常训练
    def __init__(
        self,
        mode="min",
        patience=10, # 1
        ...
        self.eps = eps
        self.last_epoch = -1
        self.continue_threshold = continue_threshold # 0.005 0.5% 如果表现低于这个阈值，那么从stop转向go，继续训练
    ...
    def _reset(self):
        ...
        self.best = self.mode_worse # -inf
        self.cooldown_counter = 0
        self.num_bad_epochs = 0
        self.in_stop = False

    def step(self, metrics, epoch=None): # metrics就是score
        # 如果不使用step方法进行更新，那么永远都会在go，就是正常训练
        ...
        if self.is_better(current, self.best): # score
            self.best = current
            self.num_bad_epochs = 0
        else:
            self.num_bad_epochs += 1

        if self.in_cooldown: # cooldown的存在增加了进入到stop的难度，使得模型大部分时间在go之中
            self.cooldown_counter -= 1
            self.num_bad_epochs = 0  # ignore any bad epochs in cooldown

        if self.num_bad_epochs > self.patience: # 10
            self.in_stop = True
    ...
    def _cmp(self, mode, threshold_mode, threshold, a, best): # a就是score，threshold应该是为了增强稳定性，给better一个空间
    ...
    def _init_is_better(self, mode, threshold, threshold_mode): # max 0.001 rel 对于是不是更好而言，必然是max
        ...
        self.is_better = partial(self._cmp, mode, threshold_mode, threshold) # 实际调用的是cmp函数，得到的结果是bool类型

    def _init_continue_is_better(self, mode, threshold, threshold_mode): # min 0.005 rel 决定是否重启就是min
        # 看是否跌落最低阈值
...
class tbLogger(object):
    def __init__(
        self,
        log_dir, # logs/bert_base_6layer_6conect-VQA_second_stage_pretrain
        txt_dir, # save/bert_base_6layer_6conect
        task_names, # ["VQA", "GQA" ...]
        task_ids, # ["TASK1"]
        ...
        self.txt_f = open(txt_dir + "/" + txt_name, "w") # save/bert_base_6layer_6conect/out.txt
        self.task_id2name = {
            ids: name.replace("+", "plus") for ids, name in zip(task_ids, task_names)
        }
        self.task_ids = task_ids
        self.task_loss = {task_id: 0 for task_id in task_ids} # ["TASK1"]
        ...
        self.task_num_iters = task_num_iters # dict()
        self.epochId = 0
    ...
    def linePlot(self, step, val, split, key, xlabel="None"): # split：train或者val，key：VQA_masked_loss_t
        if self.save_logger:
            self.logger.add_scalar(split + "/" + key, val, step) # 变量显示的名字，y值，x值
            # train/VQA_masked_loss_t
    ...
    def step_train_CC( # 记录每一个step的loss， 用于画图
        self,
        epochId,
        stepId, # 这个就是具体哪一个iter
        masked_loss_t,
        masked_loss_v,
        next_sentence_loss,
        norm, # ?
        ...
        self.task_step_tmp[task_id] += self.gradient_accumulation_steps
        self.epochId = epochId
        total_loss_train = masked_loss_t + masked_loss_v + next_sentence_loss
        ...
        self.linePlot(stepId, total_loss_train, split, self.task_id2name[task_id] + "_total_loss")

    def step_val(self, epochId, loss, score, task_id, batch_size, split): # 也是没有画图...
    ...
    def step_val_CC( # 因为val看一个个step的话，没办法看出来整个batch的训练情况，所以这里没画图，放在后面了
        self,
        epochId,
        masked_loss_t,
        masked_loss_v,
    ...
    def showLossValCC(self, iterID, task_stop_controller=None): # val loss画总的才有意义
        progressInfo = "Eval Ep: %d " % self.epochId
        lossInfo = "Validation "
        total_loss_val = 0
        for task_id in self.task_ids: # TASK1 TASK2 TASK12
            ...
            total_loss_val = masked_t_loss_val + masked_v_loss_val + next_sentense_loss_val
            lossInfo += "[%s]: Masked_t %.3f Masked_v %.3f NSP %.3f Total %.3f" % (
                self.task_id2name[task_id],
                masked_t_loss_val,
                masked_v_loss_val,
            ...
            self.linePlot(
                ...
                masked_v_loss_val,
                "val",
                self.task_id2name[task_id] + "_mask_v",
            )
            self.linePlot(
                iterID,
                next_sentense_loss_val,
            ...
            self.linePlot(
                ...
                total_loss_val,
                "val",
                self.task_id2name[task_id] + "_total",
            )
            if task_stop_controller is not None:
                self.linePlot(
                    self.task_step[task_id],
    ...
    def showLossTrainCC(self): # 这个是几个step下，平均值；比如N个step print一次loss，这个loss就是N个step的平均值
        # show the current loss, once showed, reset the loss.
        lossInfo = ""
        for task_id in self.task_ids:
            if self.task_num_iters[task_id] > 0:
                if self.task_step_tmp[task_id]:
                    lossInfo += (
                        "[%s]: Iter %d Ep: %.2f Masked_t %.3f Masked_v %.3f NSP %.3f lr %.6g Total %.3f"
                        % (
                            self.task_id2name[task_id],
                            self.task_step[task_id],
                            self.task_step[task_id]
                            ...
                            self.task_norm_tmp[task_id]
                            / float(self.task_step_tmp[task_id]),
                            (self.masked_t_loss[task_id] + self.masked_v_loss[task_id] + self.next_sentense_loss[task_id])
                            / float(self.task_step_tmp[task_id]),
                        )
                    )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if pretrained_model_name_or_path in cls.pretrained_model_archive_map:
            ...
            if from_tf:
                ...
                archive_file = pretrained_model_name_or_path # pretrain/pretrain_model.bin
