...
class ConceptCapLoaderTrain(object):
    ...
    def __init__(
        self,
        corpus_path, # file_path default = data/conceptual_caption/
        tokenizer, # tokenizer = BertTokenizer.from_pretrained
        bert_model, #  default="bert-base-uncased"
        seq_len, # max_seq_length， default = 36
        encoding="utf-8",
        visual_target=0,
        hard_negative=False,
        batch_size=512,
        ...
        preprocess_function = BertPreprocessBatch(
            ...
            seq_len, # 36
            36, # region_len
            self.num_dataset, # data_size
            encoding="utf-8",
            visual_target=visual_target, # 0
            objective=objective,
        )
        ...
        self.ds = td.BatchData(ds, batch_size) # 这部分中，数据已经装填好了，所以才有下面的内容
        # self.ds = ds
        self.ds.reset_state()
    ...
    def __iter__(self):
        ...
        for batch in self.ds.get_data(): # 装填好的数据中，直接读取batch
...
class ConceptCapLoaderVal(object):
    ...
    def __iter__(self):
        for batch in self.ds.get_data():
            ...
            batch_size = input_ids.shape[0]
            sum_count = np.sum(masked_label == 0, axis=1, keepdims=True) # 1被遮住，0不被遮住
            sum_count[sum_count == 0] = 1
            g_image_feat = np.sum(image_feat, axis=1) / sum_count
            image_feat = np.concatenate(
                [np.expand_dims(g_image_feat, axis=1), image_feat], axis=1
...
class BertPreprocessBatch(object): # 将原数据内容生成NSP和MLM的内容，前面是加载原本数据，现在是BERT化
    ...
    def __call__(self, data):
        ...
        image_location = np.zeros((self.region_len, 5), dtype=np.float32) # location是5维向量

        # calculate the IOU here. 这里iou的计算是没有归一化的坐标
        overlaps = iou(image_location_wp, image_location_wp) # 抽取的坐标直接送进iou之中
    ...
    def convert_example_to_features( # 将原本内容变成BERT形制的feature，例如加special token，例如pad到同一长度
        ...
        tokens = tokenizer.add_special_tokens_single_sentence(tokens) # 加上CLS 和 SEP
    ...
    def random_region(self, image_feat, image_loc, num_boxes, is_next, overlaps):
        ...
        """
        output_label = []
        masked_label = np.zeros((image_feat.shape[0])) # 特征的个数，bb的个数，每一个暂时对应0

        for i in range(num_boxes): # 对于每一个region；这里要注意，在iou应该置零的操作之下，还是会有应该置零的bbox进入for循环
            # 这时候就需要使用masked label进行控制，这也是利用了或运算的性质，一旦有1出现，就不会再变成0，代表应该因为iou
            # 而置零的bbox信息会被存储下来
            prob = random.random()
            # mask token with 15% probability
        ...
        return image_feat, image_loc, output_label, masked_label # image_feat, image_loc, image_label, masked_label
        # 在output label的大条件下，还需要加上masked label里面的image feature置零
