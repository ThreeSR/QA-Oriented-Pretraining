...
class ImageFeaturesH5Reader(object):
    ...
    def __getitem__(self, image_id):
        # ************************************************************************************** #
        if not hasattr(self, '_image_ids'):
            self.open_lmdb()
        # ************************************************************************************** #
        ...
        if self._in_memory:
            ...
            if self.features[index] is not None:
                ...
                with self.env.begin(write=False) as txn:
                    ...
                    image_location[:, :4] = boxes # 四个坐标
                    image_location[:, 4] = (
                        (image_location[:, 3] - image_location[:, 1])
                        * (image_location[:, 2] - image_location[:, 0])
                        / (float(image_w) * float(image_h)) # 面积归一化？？加速收敛？？
                    )

                    image_location_ori = copy.deepcopy(image_location)
                    # 整张图都做归一化处理了，这样子更加快收敛，和上面的面积是互相对应的
                    # 应该就是为了加快收敛，因为每个detect到的图块是不同的，所以需要统一标准
                    ...
                    self.boxes[index] = image_location # 新的location
            ...
            with self.env.begin(write=False) as txn:
                item = pickle.loads(txn.get(image_id))
                image_id = item["image_id"]
                image_h = int(item["image_h"]) # 图像原本大小
                image_w = int(item["image_w"])
                # num_boxes = int(item['num_boxes'])
                ...
                num_boxes = features.shape[0] # 100
                g_feat = np.sum(features, axis=0) / num_boxes
                num_boxes = num_boxes + 1 # 101
                features = np.concatenate(
                    [np.expand_dims(g_feat, axis=0), features], axis=0
                )
                ...
                ) # np.expand_dims使用的原因是，image_location是N * 5的大小，bbox数量，所以为了可以
                # concatenate，有了这个使用，可能这个G只是保留一定信息的用途
                # 猜测：这里g-location应该就是为了IMG token留位置的，这样一来，经过这个函数，img feature不需要再添加token了

        return features, num_boxes, image_location, image_location_ori # 101 bboxes
