...
# This file is used for second pre-train only.
...
import pdb
from vilbert.datasets.vqa_dataset_second_pretrain import VQALoader
...
def ForwardModelsTrain(
    ...
    # get the batch
    batch = task_iter_train[task_id].next() # 一个个读取batch
    batch = tuple(t.cuda(device=device, non_blocking=True) for t in batch)
