...
class FeatureExtractor:
    ...
    def _process_feature_extraction(
        ...
        for i in range(batch_size):
            ...
            for cls_ind in range(start_index, scores.shape[1]):
                cls_scores = scores[:, cls_ind] # 这个就是分出来的物体类别的分数，是概率值，经过了softmax
    ...
    def _save_feature(self, file_name, feature, info):
        file_base_name = os.path.basename(file_name) # basename就是一个文件路径的最后文件名部分，比如/home/.../file.txt的file.txt
        file_base_name = file_base_name.split(".")[0]
        info["image_id"] = file_base_name
        info["features"] = feature.cpu().numpy()
        file_base_name = file_base_name + ".npy"

        np.save(os.path.join(self.args.output_folder, file_base_name), info) # 最后保存的内容是info
